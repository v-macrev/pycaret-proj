...
def gera_proj_seg(lista_consulta, n, model_folder):
    ...
    def consulta_db():
        try:
            with connections.cursor() as cursor:
                ...
                logger.info("Starting the cursor for consulta_db")
            ...
            cursor.close()
            logger.info("Closing the cursor for consulta_db")
    ...
    print(opcao_selecionada_molecula)
    query = f"""select
                    FF.dt_periodo,
                    VERT.tp_mercado,
    ...
    print(query)
    try:
        logger.info("Starting the cursor for query")
        with connections.cursor() as cursor:
            cursor.execute(query)
            results = cursor.fetchall()
            columns = [col[0] for col in cursor.description]
        ...
        logger.error("Error executing query: %s", e)
            ...
            cursor.close()
            logger.info("closing the cursor for query")
    ...
    def gera_prev(df, nome_prev):
        ...
        for i in lista_cols:
            ...
            try:
                modelo_salvo = joblib.load(f"{model_folder}/seg_{nome_prev}_{i}_{molecula}_{month}_{year}.joblib")
                logger.info(f"Usando modelo salvo: seg_{nome_prev}_{i}_{molecula}_{month}_{year}.joblib")
                ...
                df_y_predito = pd.concat([df_y_predito, y_predito.set_index('dt_periodo')], axis=1, join='outer')
                ...
                # Salvando modelo
                model_filename = f"{model_folder}/seg_{nome_prev}_{i}_{molecula}_{month}_{year}.joblib"
                joblib.dump(modelo_final, model_filename)
                logger.info(f"Modelo seg_{nome_prev}_{i}_{molecula}_{month}_{year}.joblib")
